...
## 2026-05-12
- **Topic-scoped engine sessions 设计** — 收敛 WT 产品方向：WT 保持单任务，不做全局任务拆分/调度；同项目多 WT 并发来自飞书话题，每个话题一个独立 WT session。设计文档 `docs/superpowers/specs/2026-05-12-topic-scoped-engine-sessions-design.md` 定义 `project_id + chat_id + thread_root_id + engine_type` 的话题级 session key，统一 Deep/Spec/WT 的话题内继续对话路由，要求 `/wt <需求目标>` 启动并绑定话题、空 `/wt` 进入目标输入或状态页，WT 状态从 `ProjectContext.worktree_state` 迁出到 topic-scoped store，merge 项目级串行，overlap 首版只提示不阻塞，WT review 复用 Spec 多角色评估思想但通过轻量适配器接入 → [详细记录](2026-05-12.md)
