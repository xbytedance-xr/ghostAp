...
## 2026-05-12
- **飞书编程卡片精简、定位标题与长任务 TTL 顺延** — 新增 `ux/card_preview.html` 精简过程卡片预览并将聚合块做成可点击展开；完成/失败工具调用从单行 markdown 改为折叠聚合面板，详情只列读/搜/改/运行输入摘要，不渲染 `tool_output`；v2 header 统一显示 Spec/Worktree 迭代轮次、任务/子任务标签、卡片序号和分页，Spec 多任务卡与 subagent 卡继承当前轮次，Worktree 执行/重试维护 `iteration_count`；CardSession TTL 不再按固定 30 分钟打断仍有活跃工具/进度的长任务，而是仅在长时间无更新且无活跃工作时回收；本轮过程全量 `6374 passed, 49 warnings` + `git diff --check` 通过 → [详细记录](2026-05-12.md)
