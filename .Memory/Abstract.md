...
- **TimeAgo 语义层落地到 ACP/Core/UI 调用图** — 将 `compute_time_ago_bucket` 明确为 TimeAgo 语义层 SSOT，新建 `format_time_ago_from_bucket` 供“bucket→文案”调用；在 `ACPSessionManager` 中为 `list_active_sessions` 增加 `idle_bucket` 与 `idle_text` 字段，并引入 `classify_idle_health(bucket)` 作为 idle 健康分类扩展点；重写 SystemHandler `show_tools_status` 内部 `_format_last_used` 逻辑，改为基于 `compute_time_ago_bucket` 的语义判断结合 `UI_TEXT` 模板渲染 last_used 文案；CoreBuilder/CardBuilder/ProjectBuilder 中所有相对时间展示路径统一收口到“timestamp→seconds→bucket→渲染”的调用链；新增/扩展 text_utils/acp_keepalive/worktree_command_routing/card 等测试，并在全量 2492 tests 上验证零回归 → [详细记录](2026-04-21.md)

- **IdleHealth Telemetry API 收紧与内部实现隐藏** — 通过 `__all__` 机制结合函数名下划线前缀（`_`），对 `src.acp.telemetry` 模块的公开 API 进行全面收紧。将 `DefaultIdleHealthTelemetry`、`IdleHealthServiceProtocol` 及各类 helper 函数降级为内部实现细节，对外仅保留 `IdleHealthConfig`、`build_idle_health_config_for_manager` 与 `IdleHealthTelemetryContext`，降低外部调用心智负担并隔离实现细节。全局更新测试 mock 路径等引用，并在全量 2602 tests 上验证零回归 → [详细记录](2026-04-21.md)
...
## 2026-04-21
- **审查异常文案本地化打磨** — 在合成超时异常的审查建议时，将底层并发工程术语（futures）的暴露替换为符合统一本地化心智要求的纯中文表达，无缝对接现有异常处理控制流并消除中英混排 → [详细记录](2026-04-21.md)
- **相对时间文案统一 & 共享文案层增强** - 收口 format_seconds_ago/format_time_ago 到单一实现，统一 ACP/卡片相对时间文案，并补充完整测试覆盖 → [.Memory/2026-04-21.md](2026-04-21.md)
...
- **IdleHealth UNKNOWN 回退日志可观测性增强** - 在 `src/acp/models.py` 定义 `IdleHealthContext` 聚合 `agent_type/session_key/chat_id/project_id/thread_id/session_id/idle_seconds/idle_bucket` 等关键信息；`ACPSessionManager.list_active_sessions` 构造并传递 `IdleHealthContext` 给 `classify_idle_health`，统一在 `_log_idle_health_classification_fallback` 中输出带上下文字段的 WARNING 日志（保持原有 bucket/error_type/error 信息不变），同时通过扩展 `tests/test_time_ago.py::TestACPSessionManagerIdleHealth` 与 `tests/test_acp_keepalive.py::TestListActiveSessionsTimeAgo` 锁定日志与上下文传递行为，并在全量 2519 tests 上验证零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
 - **IdleHealth Telemetry 抽象与可注入监控/日志收口** — 在 `src/acp/manager.py` 中引入 `IdleHealthTelemetry` 协议与 `DefaultIdleHealthTelemetry` 默认实现，将 IdleHealth UNKNOWN 回退场景下的监控和日志行为收口到可注入的 telemetry 对象；`ACPSessionManager.__init__` 新增 `idle_health_telemetry` 可选参数并在实例上持有 `_idle_health_telemetry`；`_classify_idle_health_with_telemetry` 实例方法在预期输入异常时通过 telemetry 统一记录 WARNING 日志与监控埋点并回退为 `IdleHealth.UNKNOWN`，`list_active_sessions` 通过该实例方法计算 `idle_health`；同时保留静态 `classify_idle_health` 与模块级 `_record_idle_health_fallback_metric`/`_log_idle_health_classification_fallback` 作为兼容入口，并更新 `tests/test_acp_keepalive.py` 与 `tests/test_time_ago.py` 用例，分别通过 telemetry 注入与静态入口验证行为不变，最终在子集与全量 pytest 上确认无回归；后续增量在 `src/acp/telemetry.py` 中强化 IdleHealth Telemetry 使用准则与统一入口，并将 `IdleHealthContext` 显式降级为 `[INTERNAL]` Telemetry-only 实现细节，仅通过别名 `IdleHealthTelemetryContext` 与 `__all__` 导出对外公开 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealth 兜底策略下沉到 Telemetry 模块统一入口** — 在 `src/acp/telemetry.py` 中新增统一入口函数 `classify_idle_health_with_fallback(bucket, context=None, telemetry=None)`，在正常路径下直接委托 `classify_idle_health_from_bucket`，仅在 ValueError/TypeError/KeyError/AttributeError 等预期输入异常时通过可注入的 `IdleHealthTelemetry` 记录 WARNING 日志与监控埋点并回退为 `IdleHealth.UNKNOWN`，其它异常原样向上传播；`ACPSessionManager._classify_idle_health_with_telemetry` 与静态 `classify_idle_health` 全部改为委托该入口，其中静态方法改为通过 Telemetry 层公开工厂 `get_manager_compat_idle_health_telemetry()` 构造专用 `IdleHealthTelemetry` 实例而不再依赖跨模块私有 hook 类，实现“业务管理层（manager）→ 遥测适配层（telemetry 工厂+协议）→ 遥测实现层（私有 hook 与具体打点）”的分层；新增 `tests/test_acp_keepalive.py::TestIdleHealthTelemetryModule` 覆盖默认 Telemetry 委托模块级 hook、模块级 hook 安全性与统一入口正常路径行为，并在 `tests/test_acp_models.py::TestIdleHealthClassificationStatic` 中调整静态入口回退测试使用 Telemetry 模块私有 hook 打桩，最终在全量 2544 tests 上确认零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
- **ACPSessionManager–Telemetry API 面收窄与 IdleHealth 使用准则模块头注释** — 在 `src/acp/telemetry.py` 中完善面向 manager 的 Telemetry 门面：`classify_idle_health_for_manager(bucket, context=None, telemetry=None)` 统一委托 `classify_idle_health_with_fallback`，在未显式注入时通过 `get_manager_compat_idle_health_telemetry()` 选择专用 Telemetry 实例；新增 `get_idle_health_telemetry_for_manager(telemetry=None)` 作为 ACPSessionManager 构造函数的默认 Telemetry 注入 helper，使 manager 只需依赖协议与门面函数而无需感知工厂细节；`ACPSessionManager.__init__` 及 `_classify_idle_health_with_telemetry` 改为分别通过 `get_idle_health_telemetry_for_manager` 和 `classify_idle_health_for_manager(..., telemetry=self._idle_health_telemetry)` 接入 Telemetry，静态 `classify_idle_health` 则收缩为对 `classify_idle_health_for_manager(bucket, context=context)` 的单行委托；在此基础上根据审计建议为 `src/acp/telemetry.py` 顶部模块 docstring 增补并后续重写「IdleHealth 使用准则」小节，将原本的 3 条 bullet 收敛为“一行角色概述 + 三行规则”，统一使用“业务调用方/ACPSessionManager/Telemetry 模块”三类主体，并显式标注 `classify_idle_health_with_fallback` 仅为 Telemetry 模块内部与测试用例使用的内部 API、不承诺向业务代码稳定兼容；通过 `python -m compileall src/acp/telemetry.py` 与 `uv run python -m pytest tests/test_acp_keepalive.py -q` 验证新增文档契约未引入任何行为回归 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealthContext 从 ACP 模型层抽离到 Telemetry 层** — 将用于 IdleHealth UNKNOWN 回退日志/监控场景的结构化上下文 `IdleHealthContext` 从 `src/acp/models.py` 迁移到 `src/acp/telemetry.py`，使 ACP 核心模型层仅承载协议事件与会话状态等领域结构，IdleHealth 上下文类型完全归属 Telemetry 层；`ACPSessionManager` 及相关测试统一通过 Telemetry 模块导入 IdleHealthContext，`list_active_sessions` 中上下文构造与传递逻辑保持不变，并通过 IdleHealth/Telemetry/ACPSessionManager/handlers 子集与全量 2553 tests 回归验证零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
...
- **session_key Lint Gate 提示文案增强（推荐修复路径 & 结构层级）** — 在既有 `tests/test_session_key_lint.py` AST Lint Gate 的基础上，为 session_key 手工解析反模式新增统一推荐修复文案常量 `RECOMMENDED_FIX_NOTE`（明确提示“检测到对 session_key 的手工解析，请改用 ACPSessionManager._parse_session_key(...) 或项目中既有的解析辅助函数，避免手写字符串拆分逻辑。”），并在 `src/utils/text.py` 中新增通用 helper `render_violation_report(...)`，将 pytest 失败输出结构统一为“标题 → 空行 → 【推荐修复方式】 → 推荐修复说明 → 空行 → 违规列表”；`_format_violations` 现通过该 helper 生成最终文案：头部只出现一次推荐修复说明，所有违规行仅包含位置与英文短原因（如 `manual parsing of session_key via split(':') is forbidden`），完全不重复长文案，既保证“提示 + 修复路径”，又保持长列表输出低噪音；新增/增强 `tests/test_session_key_lint.py` 与 `tests/test_text_utils.py::TestRenderViolationReport` 覆盖新结构与降级行为，并在全量 pytest 上确认零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
 - **IdleHealthService 协作者依赖注入与协议化收口** — 在 `src/acp/telemetry.py` 中引入 `IdleHealthServiceProtocol` 协议与工厂 `get_idle_health_service_for_manager(idle_health_service=None, *, telemetry=None)`，将“基于会话快照计算 IdleHealth + TimeAgoBucket + Telemetry-only 上下文”的职责收口到可注入协作者对象；`ACPSessionManager.__init__` 新增 `idle_health_service` 可选参数并通过工厂完成注入或默认构造，`list_active_sessions` 在遍历 `_sessions` 时统一调用 `_idle_health_service.classify_session_idle_health(...)` 获取 `idle_seconds`/`idle_bucket`/`idle_health` 与 `IdleHealthTelemetryContext`，manager 自身不再直接拼装上下文或调用 Telemetry 兜底入口；在 `tests/test_acp_keepalive.py` 中通过 Dummy/Fake IdleHealthService 验证注入路径以及返回结构的可控性，在 `tests/test_acp_models.py` 中为工厂函数补充“显式实例优先/缺省构造默认实现”两类单测，并在全量 2566 tests 上确认零回归 → [.Memory/2026-04-21.md](2026-04-21.md)

## 2026-04-21
- **审查异常文案本地化打磨** — 在合成超时异常的审查建议时，将底层并发工程术语（futures）的暴露替换为符合统一本地化心智要求的纯中文表达，无缝对接现有异常处理控制流并消除中英混排 → [详细记录](2026-04-21.md)
- **共享文案层增强 & 相对时间文案统一** — 统一 `format_time_ago`/`format_seconds_ago` 文案入口，收口 ACP/卡片相对时间实现 → [.Memory/2026-04-21.md](2026-04-21.md)
- **ACPSessionManager session_key 协议收口与 Lint Gate** — 收紧 session_key 编解码协议并通过 pytest+AST 静态检查禁止手工解析 → [.Memory/2026-04-21.md](2026-04-21.md)
- **ACP 协议模型 vs 展示模型边界收紧** — 将 ACP 工具/模型视图型数据从 acp.models 迁移到 ttadk.models/card.models，强化协议层与 UI 层分界 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealth Telemetry 抽象与兜底策略统一入口** — 将 IdleHealth UNKNOWN 回退策略与可观测性收口到 telemetry 模块统一入口；ACPSessionManager 仅依赖门面与协议 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealthContext Telemetry-only 命名心智模型收紧** — 强化 `IdleHealthContext` docstring，将其首行标注为 `[INTERNAL]` 并强调“仅用于 IdleHealth UNKNOWN 回退场景的日志/监控上下文，不参与协议/业务建模”；在 Telemetry 模块内保留 `IdleHealthTelemetryContext = IdleHealthContext` 语义别名，并通过调整 `src/acp/telemetry.py.__all__` 仅导出 `IdleHealthTelemetryContext`（不导出 `IdleHealthContext` 本身），配合新增 `tests/test_acp_models.py::TestTelemetryPublicExports::test_star_import_exports_telemetry_context_only` 单测，将“IdleHealthContext 降级为 Telemetry 内部实现细节、仅通过别名对外暴露 Telemetry-only 语义”的约束编码为可回归契约 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealthTelemetryContext 注释语义前移（Telemetry-only 心智模型收紧）** — 在 `src/acp/telemetry.py` 中为对外公开的 Telemetry-only 别名 `IdleHealthTelemetryContext = IdleHealthContext` 增加变量级 docstring，并根据审计建议将首行收敛为一行紧凑标签+用途说明：`【Telemetry-only · UNKNOWN 回退】IdleHealth UNKNOWN 回退路径的日志/监控上下文（不得用于业务协议或领域模型）`；后续行简要说明其用于 UNKNOWN 回退路径中的结构化日志/监控上下文，字段语义与 `IdleHealthContext` 保持一致，由 Telemetry 层消费；结合既有 `IdleHealthContext` `[INTERNAL]` docstring 与 `__all__` 导出策略，使调用方在 IDE 补全/悬浮提示中第一眼就建立 Telemetry-only 心智模型，同时通过 IdleHealth/Telemetry/ACP 子集与全量 pytest 确认本次为纯文档增强零行为回归 → [.Memory/2026-04-21.md](2026-04-21.md)
- **IdleHealthConfig 依赖解析下沉至 Telemetry 层统一入口** — 在 `src/acp/telemetry.py` 中为 `IdleHealthConfig` 新增 `resolve_for_manager(...)` 类方法，集中实现 `ACPSessionManager` 构造过程中的 IdleHealth 协作者解析逻辑；内部按“显式构造参数 > 配置对象字段 > Telemetry 工厂默认实现”的优先级返回 `(idle_health_telemetry, session_telemetry, idle_health_service)` 三元组，`ACPSessionManager.__init__` 仅需一次性委托该入口完成依赖注入，显著降低构造函数内的分支复杂度，并将 IdleHealth wiring 心智统一收口到 Telemetry 模块 → [.Memory/2026-04-21.md](2026-04-21.md)
- **ACPSessionManager IdleHealth 构造面收紧与显式参数软弃用** — 在 `src/acp/manager.py` 中将 `idle_health_telemetry` / `session_telemetry` / `idle_health_service` 三个协作者参数标记为 [DEPRECATED] 兼容入口，并在检测到显式传入时输出一次性 WARNING，引导调用方迁移至 `idle_health_config=IdleHealthConfig(...)` 或 `build_idle_health_config_for_manager()` 统一注入路径；同步将 `src/feishu/ws_client.py` 中各 ACPSessionManager 构造收口到单一 `IdleHealthConfig` 实例注入，真正让 Telemetry 层成为 IdleHealth wiring 的 SSOT；通过 `tests/test_acp_keepalive.py::TestManagerIdleHealthDeprecation` 与 `tests/test_feishu_ws_client_idle_health_config.py` 等用例锁定软弃用告警与行为兼容，并在全量 2576 tests 上确认零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
 
   - **Worktree WS 拦截逻辑接入旅程状态机** — 将 Worktree “是否等待 goal” 判定从 `FeishuWSClient` 内部的 enabled/ready 组合条件下沉到 `WorktreeManager.is_awaiting_goal(state)`，以 WorktreeJourneyStatus（PENDING/AUTO_EXECUTING）+ ready 单元为唯一判断条件；`ws_client._is_worktree_awaiting_goal` 与 SMART 模式 `_process_with_intent` 路由逻辑统一依赖该领域 helper，并通过 WorktreeManager/WS 路由相关新增单测与定向 pytest 子集验证，确保自动执行路径与普通 SMART 消息在新状态机接入下行为零回归 → [.Memory/2026-04-21.md](2026-04-21.md)
## 2026-04-02
