...
## 2026-05-12
- **飞书编程卡片精简、定位标题、Help 移动端入口与长任务 TTL 顺延** — 新增 `ux/card_preview.html` 精简过程卡片预览并将聚合块做成可点击展开；完成/失败工具调用从单行 markdown 改为折叠聚合面板，详情只列读/搜/改/运行输入摘要，不渲染 `tool_output`；Spec/Worktree 有迭代的卡片标题收窄为轮次，任务/子任务、卡片序号和分页下沉到 subtitle/footer，Spec 多任务卡与 subagent 卡继承当前轮次，Worktree 执行/重试维护 `iteration_count`；`/help` 常用入口从竖排大按钮改为两列小按钮，状态区去掉重复长路径；CardSession TTL 不再按固定 30 分钟打断仍有活跃工具/进度的长任务，而是仅在长时间无更新且无活跃工作时回收；本轮过程全量 `6375 passed, 49 warnings`，卡片/header/help 子集 `182 passed`，`git diff --check` 通过 → [详细记录](2026-05-12.md)
