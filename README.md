...
飞书 AI 开发助手 —— 通过飞书对话实现安全的远程 Shell 执行、多模型 AI 编程（Coco / Claude / Aiden / Codex / Gemini / TTADK）、以及自主任务编排（Deep / Spec / Worktree / Slock 引擎）。无需公网 IP，WebSocket 长连接即插即用。

## 功能概览

GhostAP 提供 **8 种交互模式** + **4 种编排引擎**，覆盖从简单命令到复杂开发任务的全场景：
...
└─────────┘ └──────────┘           └─────────────────┘
┌─────────┐ ┌──────────┐           ┌─────────────────┐
│ Gemini  │ │  TTADK   │           │  Slock Engine   │
│ 多轮编程 │ │ 多工具编程 │           │  多 Agent 团队协作 │
└─────────┘ └──────────┘           └─────────────────┘
```
...
| **Slock** | 在飞书群内创建多 Agent 团队，按角色/技能自动分配任务 | `/slock` 或 `/new-team <名称>` | `/slock stop` 或 `/team dissolve <名称>` |

#### 编排引擎对比

| 维度 | Deep Engine | Spec Engine | Worktree Engine | Slock Engine |
|------|------------|-------------|-----------------|--------------|
| 执行策略 | 一次规划，按依赖推进，可委托并行子任务 | `Spec→Plan→Task→Build→Review` 循环，按依赖推进 | 并行多工具分支执行 | 飞书群内多 Agent 团队协作 |
| 适用场景 | 目标明确的单次冲刺 | 需要"可复盘产物"驱动持续推进 | 多工具协作、并行开发 | 长期团队角色、跨任务记忆、技能沉淀 |
| 进度追踪 | 计划条目 + 工具调用 | 阶段产物（Spec/Plan/Task）+ 验收标准 + 审查 | 工具选择 + 分支进度 | Agent 状态 + Kanban 任务看板 |
| 终止条件 | 执行完毕/手动停止 | 标准满足+审查通过/收敛/手动停止/待澄清 | 所有工具完成/手动停止 | `/slock stop` 或团队解散 |
    ...
    → TaskScheduler.submit（per-chat 有序、全局并发控制）
      → IntentRecognizer（ReAct LLM 意图分类，~30 种意图）
        → 路由分发:
            ...
            SLOCK_CMD     → SlockEngine（多 Agent 团队协作）
...
| `src/feishu/` | 飞书集成：WebSocket 客户端 + Handler 层 + Renderer 层 + 消息缓存 + 路由 + 控制平面 |
...
| `src/slock_engine/` | Slock 引擎：飞书群内多 Agent 团队、角色/任务/记忆/技能画像管理 |
| `src/card/` | 卡片系统：CardSession 事件管线、纯 render、delivery 投递、actions 路由、静态 builders、主题/UI 文本 |
...
- **事件驱动渲染**: Handler/engine 产生 `CardEvent`，`CardSession` 归约状态并经纯 render + delivery 管线投递飞书卡片
- **Slock mouthpiece 协作**: 飞书群作为团队入口，角色 Agent 通过统一口径回报进度，任务看板和记忆落在项目本地 `.ghostap/slock/`
...
| `/new-team <名称>` | 创建新的飞书协作群并激活 Slock |
| `/team list` | 查看所有活跃团队 |
| `/team status <名称>` | 查看指定团队状态 |
| `/team dissolve <名称>` | 停止并移除指定团队 |
| `/new-role <名称> [--tool codex] [--model <模型>] [--role <角色>] [--prompt <指令>]` | 创建虚拟 Agent |
...
```text
/slock
/new-role Coder --tool codex --role coder
/new-role Reviewer --template reviewer
...
│   │   └── reporter.py              #   执行报告
│   │
│   ├── slock_engine/                # Slock 多 Agent 团队协作引擎
│   │   ├── engine.py                #   任务分配、Agent 调度、生命周期
│   │   ├── manager.py               #   团队恢复、跨群查找、managed chat
│   │   ├── memory_manager.py        #   L1/L2/L3 记忆、技能画像、任务看板
│   │   ├── agent_registry.py        #   跨团队 Agent 身份注册
│   │   ├── task_router.py           #   技能匹配与任务 claim
│   │   └── card_templates.py        #   状态卡和任务看板
│   │
│   ├── feishu/                      # 飞书集成
...
│   │
│   ├── card/                        # 卡片系统
│   │   ├── builder.py               #   静态卡片 facade（engine/progress 走 CardSession）
│   │   ├── actions/                 #   卡片 action 分发与路由
│   │   ├── delivery/                #   CardDelivery 统一投递引擎
│   │   ├── events/                  #   CardEvent 类型、payload、ACP/worktree adapter
│   │   ├── render/                  #   纯渲染：layout、sticky head、tools、reasoning、pagination
...
├── docs/                            # 当前架构/ADR/接入指南
│   ├── adr-*.md                     #   架构决策记录
│   ├── acp_provider_guide.md        #   ACP Provider 接入指南
│   ├── commit-message-guidelines.md #   提交信息规范
│   └── 2026-05-11-refactoring-analysis.md # 最新重构审计矩阵
├── .Memory/                         # 项目记忆（开发决策日志）
├── AGENTS.md                        # AI Agent 项目指令
├── CLAUDE.md                        # Claude Code 轻入口，指向 AGENTS.md
