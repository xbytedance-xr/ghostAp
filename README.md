...
飞书 AI 开发助手 —— 通过飞书对话实现安全的远程 Shell 执行、多模型 AI 编程（Coco / Claude / Aiden / Codex / Gemini / TTADK）、以及自主任务编排（Deep / Loop / Spec 引擎）。无需公网 IP，WebSocket 长连接即插即用。
...
| `src/loop_engine/` | Loop 引擎：迭代闭环、验收标准、收敛检测、多视角审查 |
...
- **策略层 × 传输层解耦**: Normal/Deep/Loop/Spec 与 ACP/CLI 传输独立演进
...
**测试覆盖**：3,857 个测试，覆盖 ACP 协议、Deep/Loop/Spec 引擎、Worktree 引擎、审查管线、锁定系统、Handler 路由、卡片渲染、安全沙箱、任务调度等全部核心模块。
