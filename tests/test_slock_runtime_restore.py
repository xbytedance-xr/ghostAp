...
def _write_marker(root_path: str, channel_id: str, data: dict) -> str:
    """Helper: write a .slock_channel.json marker under {root_path}/slock/{channel_id}/."""
    channel_dir = os.path.join(root_path, "slock", channel_id)
...
class TestRestoreFromDiskHappyPath:
    ...
    def test_restore_multiple_engines(self, tmp_path):
        ...
        for i, name in enumerate(["Alpha", "Beta", "Gamma"]):
            _write_marker(root, f"oc_chat_{i}", {
...
class TestRestoreFromDiskErrorHandling:
    def test_skips_corrupted_marker(self, tmp_path):
        root = str(tmp_path)
        channel_dir = os.path.join(root, "slock", "oc_bad")
    ...
    def test_empty_slock_dir_returns_zero(self, tmp_path):
        os.makedirs(os.path.join(str(tmp_path), "slock"))
        manager = SlockEngineManager()
        restored = manager.restore_from_disk(str(tmp_path))
        assert restored == 0
    ...
    def test_skips_dir_without_marker(self, tmp_path):
        root = str(tmp_path)
        os.makedirs(os.path.join(root, "slock", "oc_no_marker"))
