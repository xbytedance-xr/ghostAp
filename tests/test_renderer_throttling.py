...
from src.loop_engine.models import LoopProject, LoopProjectStatus, IterationRecord, IterationStatus
...
class TestRendererThrottling(unittest.TestCase):
    ...
    def test_deep_renderer_throttling(self):
        ...
        callbacks = renderer.create_deep_callbacks("msg_id", "chat_id", mock_project, initial_message_id="existing_msg_id")
        ...
        with patch('src.feishu.renderers.deep_renderer.CardBuilder') as mock_builder:
            ...
            self.mock_handler.patch_message.assert_called_with(
                "existing_msg_id", "{}", max_retries=1, throttle=True
            )
    ...
    def test_deep_renderer_critical_update(self):
        ...
        with patch('src.feishu.renderers.deep_renderer.CardBuilder') as mock_builder:
            ...
            self.mock_handler.patch_message.assert_called_with(
                "reply_msg_id", "{}", max_retries=1, throttle=False
            )
    ...
    def test_help_card_structure(self):
        ...
        text_elem = next((e for e in elements if e.get("tag") == "markdown" and "编程模式" in e.get("content", "")), None)
