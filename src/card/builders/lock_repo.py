    ...
    *repo_token* is an opaque identifier used in the force-release button
    ...
    if idle_timeout_seconds is not None:
        ...
        try:
            ...
            idle_timeout_min = get_settings().repo_lock_idle_timeout // 60
        except Exception:
            idle_timeout_min = 5
    ...
    if last_active_time_monotonic > 0 and (now - last_active_time_monotonic) < active_threshold:
        status_hint = UI_TEXT["repo_lock_hint_active"].format(timeout_min=idle_timeout_min)
    else:
    ...
    same_sender_line = UI_TEXT["repo_lock_same_sender_hint"].format(chat_hint=chat_hint) if is_same_sender else ""
    ...
    if command_text and len(command_text) <= MAX_COMMAND_TEXT_LENGTH:
        ...
        markdown += UI_TEXT["repo_lock_cmd_too_long"]
    if app_id:
        buttons.append({
            "tag": "button",
...
def build_lock_reclaim_notify_card(
    ...
    *,
    ...
    if hard_timeout_seconds is not None:
        ...
        try:
            from ...config import get_settings
            max_hours = max(1, get_settings().repo_lock_hard_timeout // 3600)
        except Exception:
            max_hours = 1
