"""Backward-compatible re-export — canonical location: src.card.session.ttl

DEPRECATED (deprecated in v0.1.0): This shim will be removed after 2026-06-01.
Import directly from ``src.card.session.ttl`` instead.
"""
...
_CANONICAL = "src.card.session.ttl"
...
def __getattr__(name: str):  # PEP 562
    if name in __all__:
        ...
        from src.card.session.ttl import TTLHandler  # noqa: F401

        _map = {"TTLHandler": TTLHandler}
        warnings.warn(
