"""Backward-compatible re-export — canonical location: src.card.session.factory

DEPRECATED (deprecated in v0.1.0): This shim will be removed after 2026-06-01.
Import directly from ``src.card.session.factory`` instead.
"""

import datetime
import warnings

__all__ = ["CardSessionFactory"]

_CANONICAL = "src.card.session.factory"
...
def __getattr__(name: str):  # PEP 562
    if name in __all__:
        ...
        from src.card.session.factory import CardSessionFactory  # noqa: F401

        _map = {"CardSessionFactory": CardSessionFactory}
        warnings.warn(
