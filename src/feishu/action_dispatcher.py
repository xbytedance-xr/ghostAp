    ...
    def dispatch(
        ...
        if action_type in self._action_registry_exact:
            try:
                ...
                logger.error(f"Action handler for '{action_type}' signature mismatch: {get_error_detail(e)}")
                raise e
        ...
        for prefix, handler in self._action_registry_prefix:
            if action_type.startswith(prefix):
                try:
                    ...
                    logger.error(f"Prefix handler for '{prefix}' signature mismatch: {get_error_detail(e)}")
                    raise e
