...
class LoopHandler(BaseHandler):
    ...
    def start_loop_engine(
        ...
        if not project:
            ...
            try:
                project, is_new = self.project_manager.get_or_create_project_for_path(
                    working_dir, chat_id
                )
                if is_new:
                    logger.info(
                        "Loop Engine 自动创建项目: %s @ %s",
        ...
        engine = self.ctx.loop_engine_manager.get_or_create(
            chat_id, root_path, engine_name=engine_name
        )

        def run_loop_engine():
            try:
                callbacks = self._create_loop_callbacks(
        ...
        try:
            ...
            logger.debug(
                "link_task失败(loop_engine_run): message_id=%s, run_id=%s, err=%s",
                message_id,
                handle.run_id,
                e,
            )
    ...
    def _create_loop_callbacks(
        ...
        def on_iteration_start(
            current: int, max_iterations: int, role_selection: RoleSelection = None
        ):
            role_info = ""
            if role_selection:
                role_info = f"\n\n{role_selection.role.emoji} **{role_selection.role.display_name}** — {role_selection.focus}"
            content = (
                reporter.format_iteration_start(current, max_iterations) + role_info
            )
            title = reporter.get_iteration_start_title(current, max_iterations)
            engine = self.ctx.loop_engine_manager.get(
                chat_id, project.root_path if project else ""
            )
            loop_project = engine.project if engine else None
            progress_bar = None
            if loop_project:
                progress_bar = reporter._make_progress_bar(
                    loop_project.satisfied_count, loop_project.total_criteria
                )
            msg_type, card_content = CardBuilder.build_deep_card(
                project=project,
                title=title,
        ...
        def on_iteration_done(iteration: int, record: IterationRecord):
            ...
            if engine and engine.project:
                ...
                progress_bar = reporter._make_progress_bar(
                    lp.satisfied_count, lp.total_criteria
                )
                msg_type, card_content = CardBuilder.build_deep_card(
                    project=project,
                    title=title,
        ...
        def on_criteria_update(loop_project: LoopProject):
            content = reporter.format_criteria_update(loop_project)
            title = reporter.get_criteria_update_title()
            progress_bar = reporter._make_progress_bar(
                loop_project.satisfied_count, loop_project.total_criteria
            )
            msg_type, card_content = CardBuilder.build_deep_card(
        ...
        return LoopEngineCallbacks(
            ...
            on_criteria_update=on_criteria_update,
    ...
    def resume_loop_engine(
        ...
        if not engine:
            paused = [
                e
                for e in self.ctx.loop_engine_manager.list_engines(chat_id)
        ...
        if (
            engine
            and engine.project
            and engine.project.status == LoopProjectStatus.PAUSED
        ):
            callbacks = self._create_loop_callbacks(
